...
Links (formally "hyperlinks") are foundational to the web.
A link points to a page or an element on a page, allowing content to be connected to other content.
These connections can be thought of as a web of content, which is why we call the web "the web."
...
- Links to the same destination should use the same link text, even when the links are on different pages.

---
title: Text Field
description: A text field allows a user to enter and edit text.
---
...
<img
	alt="A diagram of the text field's various components."
	src={useBaseUrl('/img/anatomy_text-field.png')}
/>

1. Label
2. Container
3. `Optional` Optional Field Indicator
4. `Optional` Help Text
5. `Optional` Icon - Left
6. Text
7. `Optional` Icon/Control - Right
    - An icon or control can go in this space. A text field can have multiple right-side icons/controls
8. `Optional` Error Text
9. `Optional` Character count
...
```jsx live
<TextField type="password" required minLength={8} maxLength={32} validateOnChange>
    Password
</TextField>
```
